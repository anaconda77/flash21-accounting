...
public enum DetailContractCategory {
    ...
    DetailContractCategory(String displayName){
    ...
    // 해당 enum 찾기
    public static DetailContractCategory fromDisplayName(String displayName){
        return Arrays.stream(DetailContractCategory.values())
                .filter(category -> category.getDisplayName().equals(displayName))
                .findFirst()
                .orElseThrow(() -> new AccountingException(DetailContractErrorCode.INVALID_CATEGORY));
    }
}
